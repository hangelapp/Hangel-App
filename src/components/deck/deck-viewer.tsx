...
/**
 * DeckViewer — Apple Keynote tarzı tam-ekran sunum oynatıcı (generic).
 *
 * Slaytları `slides` prop'undan alır; conference-deck (gelir modeli) ve Bergama
 * sunumları aynı bileşeni kullanır. Navigasyon: klavye (← → Space Esc), swipe,
 * ilerleme çubuğu, sağ/sol tıklama, tam ekran. Bazı list slaytları "reveal"
 * (build) içerir: başlık önce, maddeler sonraki tıkta.
 *
 * Opsiyonel: onDownloadPptx (PPTX indir butonu), editHref (süper-admin düzenle).
 */
...
import { type Slide, DEFAULT_QR_CAPTION_CLOSING, DEFAULT_QR_CAPTION_THANKS } from '@/lib/conference-deck';
...
export function DeckViewer({
  ...
  qrUrl,
  exitHref,
  cornerQrCaption = 'Okut, katıl',
  onDownloadPptx,
  editHref,
}: {
  slides: Slide[];
  /** Her slaytın sol alt köşesinde ve varsayılan closing/thanks QR'ında kullanılır. */
  qrUrl: string;
  /** Kapat/çıkış yapılınca gidilecek sayfa. */
  exitHref: string;
  cornerQrCaption?: string;
  /** Verilirse PPTX indir butonu gösterilir. */
  onDownloadPptx?: () => Promise<void> | void;
  /** Verilirse (süper-admin) düzenle butonu gösterilir. */
  editHref?: string;
}) {
  const router = useRouter();
  const SLIDES = useMemo(() => (slides.length > 0 ? slides : []), [slides]);
  ...
  const handleDl = useCallback(async () => {
    if (!onDownloadPptx) return;
    setDl(true);
    try { await onDownloadPptx(); } finally { setDl(false); }
  }, [onDownloadPptx]);
    ...
    >
      ...
      {slide.kind !== 'closing' && slide.kind !== 'thanks' && (
        <div className="pointer-events-none absolute bottom-6 left-5 z-40 flex flex-col items-center gap-1.5">
          <SlideQR url={qrUrl} size={isFs ? 100 : 80} />
          <span className="rounded-full bg-black/30 px-2 py-0.5 text-[10px] font-bold text-white backdrop-blur">{cornerQrCaption}</span>
        </div>
      )}
        ...
        <div className="mx-auto w-full max-w-4xl text-center">
          ...
          {slide.kind === 'title' && (
            <>
              ...
              {slide.foot && <p className="mt-10 max-w-2xl text-xs font-medium leading-snug text-white/70 sm:text-sm">{slide.foot}</p>}
            </>
          )}
          ...
          {slide.kind === 'list' && slide.hero && (
            <>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.15em] backdrop-blur">
                <Sparkles className="h-3.5 w-3.5" /> Sosyal İnovasyon
              </div>
              <h2 className="text-7xl font-black leading-none tracking-tighter sm:text-9xl">{slide.title}</h2>
          ...
          {slide.kind === 'research' && (
            <>
              ...
              <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
                {slide.unis.map((u, k) => <span key={k} className="rounded-full border px-3 py-1 text-xs font-semibold" style={{ borderColor: `${CORAL}66`, color: '#ffd9ce' }}>{u}</span>)}
              </div>
          ...
          {slide.kind === 'closing' && (
            <>
              ...
              {slide.qr ? (
                ...
                <div className="mt-10">
                  <button onClick={() => router.push(exitHref)} className="inline-flex items-center gap-2 rounded-full bg-white px-8 py-4 text-lg font-bold text-[#c5391b] shadow-xl transition active:scale-95">
                    <Sparkles className="h-5 w-5" /> Başvuruda bulun
                  </button>
                </div>
              )}
            </>
      ...
      {/* Sağ alt köşe — tam ekranda DEĞİLKEN: (opsiyonel) PPTX indir + tam ekran + düzenle */}
      {!isFs && (
        <div className="absolute bottom-7 right-5 z-40 flex items-center gap-2">
          {onDownloadPptx && (
            <button
              onClick={(e) => { e.stopPropagation(); void handleDl(); }}
              disabled={dl}

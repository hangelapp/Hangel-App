...
export default function AppBottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 mx-auto grid h-20 max-w-md grid-cols-4 border-t bg-background/80 backdrop-blur-sm">
      {navItems.map((item) => {
        ...
        return (
          <Link
            ...
            key={item.label}
            className={cn(
              "flex flex-col items-center justify-center gap-1 p-2 text-center text-muted-foreground transition-colors hover:text-foreground",
              isActive && "text-ring"
            )}
          >

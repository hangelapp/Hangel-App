...
/**
 * VolunteeringPoints — çok noktalı (multi-location) gönüllülük ilanlarında tüm
 * katılım noktalarını kart listesi olarak gösterir. Tek konumlu ilanlarda
 * opp.points boş/tanımsız olduğundan hiçbir şey render edilmez.
 *
 * Her nokta kartı: ad + tam adres (ilçe/il) + ev sahibi STK ve aksiyon satırı:
 *  - Başvur / Katıl → /volunteering/{oppId}?point={id} (detay sayfası başvuruyu üstlenir)
 *  - Kayıt QR → LogoQr (nokta atıflı, her nokta için farklı link)
 *  - Paylaş → ShareButtons (önceden yazılmış TR metin + WhatsApp/Telegram)
 *  - Yol tarifi → Google Maps (mapsUrl → lat/lon → adres)
 *
 * event-points.tsx (EventPoints) deseninden gönüllülük URL'lerine uyarlanmıştır.
 */
...
import type { Volunteering, EventPoint } from '@/lib/types';

/** Nokta için Google Maps yol tarifi linki: önce mapsUrl, yoksa lat/lon, yoksa adres. */
...
export function VolunteeringPoints({ opp }: { opp: Volunteering }) {
  // Çok-noktalı alan `sites`; `points` (number) ödül puanı olduğundan çakışmasın diye ayrıdır.
  const points = opp.sites;
  ...
  return (
    <Card className="glass-surface rounded-3xl border-white/40 shadow-sm">
      ...
      <CardContent className="space-y-4">
        {points.map((point) => {
          // Nokta atıflı gönüllülük başvuru/paylaşım linki — detay sayfası ?point ile açılır.
          const pointPath = `/volunteering/${opp.id}?point=${encodeURIComponent(point.id)}`;
          const pointUrl = `https://hangel.org/volunteering/${opp.id}?point=${encodeURIComponent(point.id)}`;
          const shareText = `${point.name} noktasında ${opp.title} için gönüllü ol! 🌍🧡 ${pointUrl} #hangel`;
            ...
            >
              ...
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  ...
                  >
                    <Link href={pointPath}>Başvur</Link>
                  </Button>
              ...
              {/* Kayıt QR — nokta atıflı (her nokta farklı) */}

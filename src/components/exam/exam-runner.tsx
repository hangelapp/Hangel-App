...
import { Loader2, Clock, GraduationCap, XCircle } from 'lucide-react';
...
export function ExamRunner({
  ...
  const submit = useCallback(
    async (finalAnswers: Record<string, number>) => {
      ...
      try {
        ...
        setResult({ passed: false, scorePct: 0, correctCount: 0, total: questions.length, attemptsLeft: 0, passPct: 0 });
      } finally {
        setSubmitting(false);
      }
    },
    [authUser, submitting, eventId, attemptId, questions.length],
  );
  ...
  // Per-soru geri sayım — süre dolunca otomatik ilerle.
  useEffect(() => {
    if (result) return;
  ...
  const pick = (optionIndex: number) => {
    ...
    setAnswers((a) => ({ ...a, [q.qid]: optionIndex }));
  };
  ...
  if (result) {
    return (
      <div className={cn('fixed inset-0 z-[2147482500] flex flex-col items-center justify-center p-6 text-white', result.passed ? 'bg-gradient-to-br from-emerald-500 to-green-600' : 'bg-gradient-to-br from-slate-700 to-slate-900')}>
  ...
  // --- Soru ekranı ---
  return (
    <div className="fixed inset-0 z-[2147482500] flex flex-col items-center justify-center bg-gradient-to-br from-indigo-600 via-indigo-500 to-violet-600 p-6 text-white">

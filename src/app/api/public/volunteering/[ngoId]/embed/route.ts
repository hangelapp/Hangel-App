/**
 * GET /api/public/volunteering/[ngoId]/embed
 *
 * STK'nın yayındaki gönüllülük ilanlarını, dış sitelere <iframe> ile gömülebilen
 * tam bağımsız HTML olarak döner (uygulama chrome'u yok). `?listing=ID` ile tek
 * ilan gömülebilir. `Content-Security-Policy: frame-ancestors *` ile global
 * X-Frame-Options bu yanıtta geçersiz kılınır (spec: frame-ancestors önceliklidir).
 */
...
export async function GET(req: Request, { params }: { params: Promise<{ ngoId: string }> }) {
  const { ngoId } = await params;
  if (!ngoId) return new Response('ngoId gerekli', { status: 400 });
  const url = new URL(req.url);
  const onlyId = url.searchParams.get('listing') || undefined;

  try {
    const { organization, items } = await fetchActiveVolunteering(ngoId, onlyId);
    const cards = items.length === 0
      ? `<p class="empty">Şu an yayında gönüllülük ilanı yok.</p>`
      : items.map((it) => `
        <a class="card" href="${escapeXml(it.url)}" target="_blank" rel="noopener">
          <div class="card-h">
            ...
            <div class="card-meta">
              <div class="org">${escapeXml(it.organization || organization)}</div>
          ...
          <div class="tags">
            ...
            ${it.applicationEnd ? `<span class="tag">Son: ${escapeXml(it.applicationEnd)}</span>` : ''}
          </div>
          <span class="cta">Başvur →</span>
        </a>`).join('');

    const html = `<!doctype html><html lang="tr"><head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1"/>
<title>${escapeXml(organization || 'hangel')} — Gönüllülük İlanları</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  <div class="head">
    ...
    <span class="sub">${escapeXml(organization || '')} · Gönüllülük</span>
  </div>

...
async function authorize(req: NextRequest): Promise<CallerContext | null> {
  ...
  try {
    ...
    // kurum yöneten kullanıcı için kritik). ngo/brand/club destekli. Caller o
    // kuruma üyeyse (managed*Id==header) ya da super-admin ise header'daki kurum
    // kullanılır; yoksa managedNgoId → managedBrandId → managedClubId sırasıyla

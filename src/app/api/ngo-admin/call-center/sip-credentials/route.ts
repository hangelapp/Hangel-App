...
async function authorize(req: NextRequest): Promise<CallerContext | null> {
  ...
  try {
    ...
    if (!d) return null;
    // Aktif kurum: üst switcher x-org-id header'ı VEYA ?ngoId= query (geriye uyum).
    const hdrId = (req.headers.get('x-org-id') || '').trim();
    const hdrKind = (req.headers.get('x-org-kind') || '').trim().toLowerCase();
    const qNgoId = (new URL(req.url).searchParams.get('ngoId') || '').trim();
    const requested = (hdrId && hdrKind === 'ngo') ? hdrId : qNgoId;
    // Super-admin'de managedNgoId olmayabilir (platform hesabı, STK panelini
    // switcher ile geziyor) — eskiden burada reddedilip panel 'Santral hazır değil'
    ...
    if (isSuper) {
      return { uid: decoded.uid, ngoId: requested || d.managedNgoId || '__platform', role: 'super-admin', email: decoded.email };
    }
    if (d.role !== 'ngo-admin') return null;
    // ngo-admin: aktif kurum kendi yönettiği STK ise onu, değilse managedNgoId'yi al.
    const ngoId = (requested && d.managedNgoId === requested) ? requested : (d.managedNgoId || '');

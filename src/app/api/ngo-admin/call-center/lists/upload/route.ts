...
async function authorize(req: NextRequest): Promise<CallerContext | null> {
  ...
  try {
    ...
    // managedBrandId/managedClubId == header, kind'e göre) ya da super-admin ise
    // header'daki kurum kullanılır; yoksa eski davranış: managedNgoId →
    // managedBrandId → managedClubId ilk dolu olan.
    ...
    if (hdrOrgId && hdrKind) {
      ...
      activeOrgId = d.managedNgoId || d.managedBrandId || d.managedClubId || undefined;
    }

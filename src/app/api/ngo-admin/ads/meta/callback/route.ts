/**
 * Meta (Facebook/Instagram) Ads bağlama — OAuth callback (Faz 1).
 *
 *   GET /api/ngo-admin/ads/meta/callback?code=&state=
 *
 * Facebook Login consent sonrası vurulur. JSON DEĞİL, HTML sayfa döner; opener'a
 * postMessage ile sonucu bildirip pencereyi kapatır (Google Ads callback deseni).
 *
 * Güvenlik:
 ...
 *    (META_OAUTH_NGO_COOKIE) verifyState ile çözülür. (Burada Firebase header
 *    yok, requireNgoAdmin kullanılamaz.)
 *  - accessToken (long-lived) yalnız Admin SDK ile metaAccounts/{ngoId}'ye
...
function htmlResponse(body: string, init?: { status?: number; clearCookies?: boolean }): Response {
  ...
  if (init?.clearCookies) {
    ...
    headers.append(
      'set-cookie',
      `${META_OAUTH_NGO_COOKIE}=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0`
    );
  }
...
function successPage(): string {
  return `<!doctype html>
<html lang="tr"><head><meta charset="utf-8"><title>Meta reklam bağlandı</title>
...
<p style="font-size:15px;line-height:1.5">Meta reklam hesabı bağlandı, bu pencereyi kapatabilirsiniz.</p>
</main>
<script>
(function(){
  try {
    if (window.opener) {
      window.opener.postMessage({ type: 'hangel-meta-connected' }, window.location.origin);
      window.close();
    }
  } catch (e) {}
})();
...
<p>Meta reklam hesabı bağlanamadı. Bu pencereyi kapatıp tekrar deneyebilirsiniz.</p>
</main>
<script>
(function(){
  try {
    if (window.opener) {
      window.opener.postMessage({ type: 'hangel-meta-error', message: ${msg} }, window.location.origin);
    }
  } catch (e) {}
})();
</script>
...
export async function GET(req: NextRequest) {
  ...
  const ngoCookie = req.cookies.get(META_OAUTH_NGO_COOKIE)?.value ?? '';
  ...
  try {
    ...
    const redirectUri = `${origin}/api/ngo-admin/ads/meta/callback`;
    ...
    // Best-effort: resolve the first accessible ad account id (non-fatal).
    ...
    if (adAccountId) docData.adAccountId = adAccountId;

    await db.collection(COLLECTIONS.metaAccounts).doc(ngoId).set(docData, { merge: true });

/**
 * Meta (Facebook/Instagram) Ads bağlama — OAuth start (Faz 1).
 *
 *   POST /api/ngo-admin/ads/meta/start
 *
 * STK admin panelinden Meta reklam hesabı bağlama akışının kickoff'u. Yetki
 * `requireNgoAdmin` scope 'ads' ile zorlanır. Config eksikse nazik 503
 * `META_NOT_CONFIGURED` döner (Google Ads start route deseninin aynısı).
 *
 * İki imzalı, time-boxed cookie yazılır:
 *  - OAUTH_STATE_COOKIE (uid taşır)    → callback double-submit + uid çözümü
 *  - META_OAUTH_NGO_COOKIE (ngoId)     → callback'te ngoId'yi güvenle çözmek için
...
export async function POST(req: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        message: 'Meta reklam bağlantısı henüz yapılandırılmadı.',
      },
      { status: 503 }
    );
  }
  ...
  const authorizeUrl = buildMetaAuthorizeUrl(config.appId, redirectUri, state);
  ...
  res.cookies.set(META_OAUTH_NGO_COOKIE, ngoCookie, {
    httpOnly: true,
    secure: true,
    sameSite: 'lax',
    path: '/',

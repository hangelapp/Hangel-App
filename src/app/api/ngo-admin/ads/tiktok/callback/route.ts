/**
 * TikTok (TikTok for Business) Ads bağlama — OAuth callback (Faz 1).
 *
 *   GET /api/ngo-admin/ads/tiktok/callback?auth_code=&state=
 *
 * TikTok for Business consent sonrası vurulur. JSON DEĞİL, HTML sayfa döner;
 * opener'a postMessage ile sonucu bildirip pencereyi kapatır (Meta Ads callback
 * deseni). TikTok kod parametresi 'auth_code' veya 'code' olabilir — ikisi de okunur.
 *
 * Güvenlik:
 ...
 *    (TIKTOK_OAUTH_NGO_COOKIE) verifyState ile çözülür. (Burada Firebase header
 *    yok, requireNgoAdmin kullanılamaz.)
 *  - accessToken (long-lived) yalnız Admin SDK ile tiktokAccounts/{ngoId}'ye
 *    yazılır; asla loglanmaz, asla client'a dönmez.
...
function htmlResponse(body: string, init?: { status?: number; clearCookies?: boolean }): Response {
  ...
  if (init?.clearCookies) {
    ...
    headers.append(
      'set-cookie',
      `${TIKTOK_OAUTH_NGO_COOKIE}=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0`
    );
  }
...
function successPage(): string {
  return `<!doctype html>
<html lang="tr"><head><meta charset="utf-8"><title>TikTok reklam bağlandı</title>
...
<p style="font-size:15px;line-height:1.5">TikTok reklam hesabı bağlandı, bu pencereyi kapatabilirsiniz.</p>
</main>
<script>
(function(){
  try {
    if (window.opener) {
      window.opener.postMessage({ type: 'hangel-tiktok-connected' }, window.location.origin);
      window.close();
    }
  } catch (e) {}
})();
...
<p>TikTok reklam hesabı bağlanamadı. Bu pencereyi kapatıp tekrar deneyebilirsiniz.</p>
</main>
<script>
(function(){
  try {
    if (window.opener) {
      window.opener.postMessage({ type: 'hangel-tiktok-error', message: ${msg} }, window.location.origin);
    }
  } catch (e) {}
})();
</script>
...
export async function GET(req: NextRequest) {
  // TikTok kod parametresi 'auth_code' veya 'code' olabilir — ikisini de oku.
  const code =
  ...
  const ngoCookie = req.cookies.get(TIKTOK_OAUTH_NGO_COOKIE)?.value ?? '';
  ...
  try {
    ...
    if (!accessToken) {
      // Without a (long-lived) access token we cannot operate later — failure.
      return htmlResponse(errorPage('token_exchange_failed'), { status: 502, clearCookies: true });
    }

    // Best-effort: resolve the first accessible advertiser id (non-fatal).
    const advertisers = await listAdvertisers(config, accessToken);
    const advertiserId = advertisers[0];
    ...
    if (advertiserId) docData.advertiserId = advertiserId;

    await db.collection(COLLECTIONS.tiktokAccounts).doc(ngoId).set(docData, { merge: true });

    ...
    return (
        <div className="space-y-4">
            ...
            <div className="max-w-md mx-auto">
                ...
                >
                    {/* Üst hangel logosu + glow */}
                    <div className="absolute top-0 right-0 w-72 h-72 rounded-full blur-3xl -translate-y-1/3 translate-x-1/3 opacity-60" style={{ backgroundColor: '#f34723' }} />
                    ...
                    <div className="relative h-full flex flex-col p-7">
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-[11px] font-black uppercase tracking-[0.3em] opacity-70">hangel</span>
                            <span className="text-[10px] font-medium opacity-50">etki hikayem</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center mt-5">
                            <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-white/50">Etki Puanı</p>
                            <p
                        ...
                        {data.highestBadgeLevel && (
                            <div className="mt-3 flex items-center justify-center gap-2 bg-white/5 backdrop-blur-md rounded-full px-4 py-2 mx-auto border border-white/10">
                                <span className="text-[10px] font-bold uppercase tracking-widest text-white/60">En Yüksek Rozet</span>
                                <span className="text-[11px] font-black" style={{ color: '#f34723' }}>{data.highestBadgeLevel}</span>
                            </div>
                        )}
                        ...
                        {allEntities.length > 0 && (
                            <div className="mt-4">
                                <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/40 text-center mb-2">
                                    Birlikte İyileştirdiğim Topluluk
                                </p>
                                <div className="flex items-center justify-center -space-x-2">
                                    {allEntities.slice(0, 6).map((e, i) => (
                                        <Avatar key={i} className="h-9 w-9 ring-2 ring-black border border-white/10 bg-white/5">
                                            <AvatarImage src={e.avatarUrl} alt={e.name || ''} />
                                            <AvatarFallback className="text-[10px] font-bold bg-white/10 text-white">
                                                {(e.name || '?').charAt(0).toUpperCase()}
                                            </AvatarFallback>
                                        </Avatar>
                                    ))}
                                    {allEntities.length > 6 && (
                                        <div className="h-9 w-9 rounded-full ring-2 ring-black bg-white/10 backdrop-blur-md flex items-center justify-center text-[10px] font-bold">
                                            +{allEntities.length - 6}
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="mt-auto pt-4 text-center">
                            <p className="text-[10px] font-medium tracking-[0.25em] text-white/40">hangel.org.tr</p>
                        </div>
                    </div>
                </div>
            </div>
...
function StatTile({ label, value }: { label: string; value: string }) {
    return (
        <div className="bg-white/5 backdrop-blur-md rounded-2xl px-3 py-2.5 border border-white/10">

/**
 * FAZ 3: 44 boş il için DIRECT URL approach.
 *
 * Strateji:
 *   1. Homepage'i hızlı yükle (1.5s wait — anti-bot menü silmeden önce)
 *   2. Tüm /Sayfalar/ linklerini scan, "İlçe" geçen ilk linki kullan
 *   3. Bulunamazsa: discover-gsb-ilce-empty bulgularındaki sabit URL ID'lerini dene
 *
...
// discover-gsb-ilce-empty çıktısından bilinen direct URL'ler
const KNOWN_DIRECT_URLS = {
  adana: '/Sayfalar/302/300/İlçe Müdürlükleri.aspx',
  bursa: '/Sayfalar/845/327/İlçe Müdürlükleri.aspx',
  antalya: '/Sayfalar/604/314/İlçe Müdürlükleri.aspx',
  aydin: '/Sayfalar/658/317/İlçe Müdürlükleri.aspx',
  denizli: '/Sayfalar/917/331/İlçe Müdürlükleri.aspx',
  hatay: '/Sayfalar/1136/344/İlçe Müdürlükleri.aspx',
};
...
// FAST link discovery — kısa wait + tüm /Sayfalar/ link'leri tara
async function findIlceUrlFast(page, base) {
  return await page.$$eval('a', (as) => {
    const all = as.map((a) => ({
      href: a.getAttribute('href') || '',
      text: (a.textContent || '').replace(/\s+/g, ' ').trim(),
    })).filter((x) => x.href);
    // Sadece /Sayfalar/ içerenler arasında "İlçe Müdür" geçeni ara
    for (const a of all) {
      if (!/\/Sayfalar\//i.test(a.href)) continue;
      if (/İlçe\s*Müdür/i.test(a.text)) return a.href;
    }
    // URL'de "İlçe" geçen
    for (const a of all) {
      if (/\/Sayfalar\/.*İl[çc]e/i.test(a.href)) return a.href;
    }
    return null;
  }).then((href) => {
    if (!href) return null;
...
async function scrapeIl(ctx, slug, name) {
  ...
  try {
    // 1. Hızlı homepage scan — anti-bot menüleri silmeden önce
    await page.goto(base + '/', { waitUntil: 'domcontentloaded', timeout: 35000 });
    await page.waitForTimeout(1500); // KISA — link DOM'da olmalı

    let url = await findIlceUrlFast(page, base);

    // 2. Fallback: bilinen direct URL
    if (!url && KNOWN_DIRECT_URLS[slug]) {
      url = base + KNOWN_DIRECT_URLS[slug];
      console.log(`  [${slug}] using known direct URL`);
    }

    if (!url) return { il: name, slug, error: 'no-ilce-link', items: [] };

    // 3. İlçe sayfası — daha uzun wait, parser sonra
...
async function main() {
  ...
  console.log(`Faz 3: ${emptyIdx.length} boş il yeniden taranıyor (hızlı discovery + direct URL)\n`);

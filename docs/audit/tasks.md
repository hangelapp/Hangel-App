...
| P2-8g | Add `useDoc` runtime `__memo` guard (mirror `useCollection:111-113`) | backend-lead | 📋 — defense-in-depth so inline `doc()` would throw same as inline `collection()` (was the original P2-8f text; renumbered after drilldown audit re-targeted P2-8f) |

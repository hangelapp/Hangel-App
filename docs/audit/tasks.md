...
| P2-7c | `src/app/**` + `src/hooks/**` collection literal migration | backend-lead | 🔧 (2026-05-18) — `src/hooks/**` 0 literal; `src/app/api/**` 16 dosya / 25 literal migrate edildi (admin/import-data, admin/messaging/{ngo-senders, ngo-wallets, pricing, whatsapp/templates/sync}, ngo-admin/messaging/{me, wallet/topup, campaigns}, messaging/{webhook/[driver], iys/export, payment/nkolay/callback, unsubscribe, csv/save, campaigns, worker/trust-score, whatsapp/webhook}). 0 atlandı; tüm literal'lar mevcut `COLLECTIONS.*` entry'lerine map'lendi (yeni entry eklenmedi). `src/app/**/page.tsx` + `layout.tsx` → P2-7c-2. Typecheck PASS, lint 0 errors (11 pre-existing warning), `tests/api/` 13/13 dosya 57 pass + 1 skip. |
| P2-7c-2 | `src/app/**/page.tsx` + `layout.tsx` collection literal migration | backend-lead | 📋 |

...
## 2026-05-23 — Disabled-account enforcement (suspend = gerçek login engeli + canlı oturum sonlandırma)
- **ID**: SEC-DISABLE-ENFORCE (yeni; tasks.md'ye eklenecek)
- **Lead**: hangel-security-lead
- **Bağlam / Tespit edilen defekt**:
  - Super-admin'in UI'da bastığı CANLI buton `handleToggleStatus` (`super-admin/users/page.tsx:109`) yalnızca `users/{uid}.status = 'Askıda'` yazıyor (client SDK `updateDoc`). Firebase Auth'a DOKUNMUYOR, `disabled` flag'i yazmıyor, mevcut `/api/admin/users/[uid]/disable` route'unu ÇAĞIRMIYOR.
  - Sonuç: bugün UI'dan "Askıya Al" → kullanıcı hâlâ giriş yapabiliyor, token yenileyebiliyor, canlı oturumu kesilmiyor. `app-shell.tsx:280` client guard'ı `.disabled === true` kontrol ediyor; `status='Askıda'` yazıldığı için guard ASLA tetiklenmiyor.
  - `_handleDisable` (underscore = dead code) gerçek disable route'unu çağırıyordu ama UI'a bağlı değil. Enable route hiç yok.
  - Disable route `revokeRefreshTokens` çağırmıyor → mevcut oturumlar `updateUser({disabled:true})` sonrası ID token süresi (max 1 saat) bitene kadar yaşayabiliyor.
- **Değişiklik (cerrahi, 3 dosya)**:
  1. `src/app/api/admin/users/[uid]/disable/route.ts`: POST body `{ action?: 'disable' | 'enable' }` (default 'disable'). disable → `updateUser(uid,{disabled:true})` + **`revokeRefreshTokens(uid)`** (YENİ — canlı oturum + token refresh anında ölür) + Firestore `disabled:true, status:'Askıda'`. enable → `updateUser(uid,{disabled:false})` + Firestore `disabled:false, status:'Aktif'` + `disabledAt/disabledBy` temizle. Super-admin auth gate + rate-limit + `{errorCode,message}` korunur.
  2. `src/app/super-admin/users/page.tsx`: `handleToggleStatus` → client `updateDoc(status)` yerine `callAdminUserRoute(uid, mode)` (mode: suspend→'disable', activate→'enable'). Böylece TEK canlı UI butonu artık Auth disable + revoke + Firestore mirror yapıyor. `callAdminUserRoute` signature `'disable'|'delete'` → `'disable'|'enable'|'delete'` genişler ve disable/enable'da `body: JSON.stringify({action})` gönderir. Toast metinleri TR korunur. Dead `_handleDisable` kaldırılır (artık canlı path bunu kapsıyor).
  3. `src/app/app-shell.tsx`: MEVCUT client guard (satır 277-297) zaten doğru ve güvenli — `userData.disabled === true` strict eşitlik. DEĞİŞİKLİK YOK gerekiyorsa minimal; guard zaten `!isUserLoading && authUser && isMounted && userData?.disabled === true` ile korunuyor. (Aşağıdaki güvenlik kanıtına bak.)
- **Client guard güvenlik yüklemi (normal kullanıcı ASLA çıkış yapmaz — kanıt)**:
  - Predicate: `(userData as {disabled?:boolean} | null)?.disabled === true`.
  - `userData` `useDoc<User>(userDocRef)` ile gelir. Yükleme/transient durumlar: `userData` `null`/`undefined` iken `?.disabled` → `undefined`; `undefined === true` → **false** → signOut ÇALIŞMAZ.
  - Alan eksikse (`disabled` doc'ta yok) → `undefined === true` → **false**. `disabled:false` → `false === true` → **false**. SADECE explicit boolean `true` signOut tetikler.
  - Effect guard'ı ek olarak `!isUserLoading && authUser && isMounted` ister → auth çözülmeden, kullanıcı yokken, mount öncesi hiç çalışmaz. userDocRef yokken `userData` `null` kalır → false.
  - Net: yalnızca giriş yapmış + doc yüklenmiş + `disabled === true` olan kullanıcı çıkış yapar. Yarış/loading/eksik-alan kombinasyonlarının hepsi false'a düşer.
- **Risk**: H (auth gating + super-admin canlı butonun davranışı değişiyor). Kötü senaryo: enable yolu bug'lı olursa super-admin yanlışlıkla disable edemez/aktive edemez — ama eski davranış zaten Auth'u hiç değiştirmediği için NET GÜVENLİK ARTIŞI. revoke tüm cihazlardaki oturumu keser → meşru kullanıcı yanlışlıkla disable edilirse enable + yeniden login gerekir (kabul edilebilir, super-admin only).
- **Blast radius**: disable route (1), super-admin users page handler (1), client guard (mevcut, dokunulmuyor/minimal). Diğer çağıran yok (`_handleDisable` dead, delete route ayrı). `firestore.rules` DOKUNULMUYOR (aşağıda öneri).
- **Test**: `npx tsc --noEmit` (tek gate, brief gereği). Davranışsal kabul: UI'dan suspend → hedef kullanıcı yeni login deneyince Firebase Auth `auth/user-disabled` döner; canlı oturumu `revokeRefreshTokens` + client guard ile kısa sürede (token refresh denemesinde ya da app-shell guard'ında) düşer.
